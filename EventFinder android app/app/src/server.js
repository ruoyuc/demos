...
const angularAppPath = path.join(__dirname, 'my-angular-app');
...
class ticketMaster {
    ...
    async search(searchrequest) {
        ...
        // const radius = Math.round(1609.34 * parseFloat(distance));
    ...
    async event(id) {
        ...
        try {
            ...
            //const json_string = JSON.stringify(Array_event);           
...
app.get('/hw9', async (req, res) => {

    const id = req.query.id;
    const eventcontent = await source.event(req.query.id);
    const result = [];
    if(eventcontent.isMusic == true){
        const artArray = eventcontent.artArray;        
        for(let i=0;i<artArray.length;i++){
            const searchResults = await source.searchArtists(artArray[i].trim());
            const artistsInfo = await source.extractArtistInfo(searchResults);
            const artist = artistsInfo[0];
            const albums = await source.getArtistAlbums(artist.id, { limit: 3});
...
function formatFollowers(number) {
    ...
    return `${formattedNumber} Followers`;
  }
